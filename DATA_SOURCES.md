...
| **OpenStreetMap (Overpass API)** | Viewport-bounded Morocco civic, transport, and landmark places | ODbL 1.0 | "© OpenStreetMap contributors" |
| **data.gov.ma (CKAN)** | Official Moroccan open-data catalog listed in the Morocco pack | Open government data; per-dataset terms | "data.gov.ma" |
| **HDX / OCHA** | Morocco humanitarian dataset titles in the Morocco pack | Per-dataset (typically CC BY) | "HDX / OCHA" |
| **Open-Meteo Air Quality** | PM2.5 / AQI sample in the Morocco pack | CC BY 4.0 | Linked "Weather data by Open-Meteo.com" |
| **Open-Meteo Forecast + Marine** | Morocco pack temperature, wind, sunrise/sunset, and coastal waves | CC BY 4.0 | Linked "Weather data by Open-Meteo.com" |
| **ONCF GTFS (unofficial)** | Morocco rail station pins in the Morocco pack | ODbL 1.0 | rail_maroc_oncf / Transitland |
| **OurAirports** | Scheduled Moroccan airports in the Morocco pack | Public domain | "OurAirports" |
| **UNESCO World Heritage** | Morocco WHC site pins | UNESCO WHC listing | whc.unesco.org state party MA |
| **AviationWeather.gov** | Nearest Morocco METAR in the pack briefing | U.S. public domain | AviationWeather.gov |
| **Aladhan Prayer Times API** | Morocco pack prayer-time sample (method 21) | Free API; provider terms | Aladhan |
| **Wikipedia geosearch** | Nearby pages in the Morocco pack sheet | CC BY-SA (article text not stored) | Wikipedia |
| **EMSC / seismicportal.eu** | Recent Morocco-region earthquakes in the pack briefing | EMSC data policy | EMSC |
...
- **Morocco pack.** The bottom-left flag opens country settings that query `/api/morocco/places` (Overpass plus ONCF GTFS stops, OurAirports scheduled airports, and UNESCO World Heritage pins; 8° bbox), `/api/morocco/context` (Open-Meteo weather/air-quality/marine, AviationWeather.gov METAR, Aladhan prayer times, Wikipedia geosearch, EMSC quakes, data.gov.ma CKAN, HDX, REST Countries / World Bank), and `/api/morocco/airport-aircraft` (5-minute cache: OpenSky bbox + adsb.lol hub snapshots for live ground tracks, plus OpenSky 8-hour arrivals/departures to park remaining aircraft on the apron as tiny `airplane.glb` models). Companion toggles enable already-shipped global layers (flights, AIS, earthquakes, FIRMS, traffic, radio) plus **ONCF trains** (`oncf-trains`): weekday (Thu 2026-09-10) and weekend (Sat 2026-09-12) templates from the public ONCF Voyages timetable, repeated indefinitely in Africa/Casablanca time. Pose walks bundled OpenStreetMap railway geometries (ODbL) between timetable stops; coach-only ODs without rail paths are omitted (geodesic fallback only for true rail pairs missing a path). Not live GPS. Close camera: tiny `train.glb`; zoomed out: sprites. There is no public Moroccan traffic-camera API comparable to Austin/TfL, Medina Bike does not publish GBFS, and Open Charge Map requires a free API key that is not wired yet.
